...
@section('content')
    ...
    <x-common.component-card title="Daftar Role">
        ...
        <div class="overflow-x-auto rounded-xl border border-gray-200 dark:border-gray-800">
            <table class="w-full min-w-[640px]"><thead class="bg-gray-50 dark:bg-gray-900"><tr>
                ...
                @foreach ($roles as $role)
                    <tr class="border-t border-gray-100 dark:border-gray-800">
                        ...
                        <td class="px-5 py-4"><div class="flex gap-2"><a href="{{ route('roles.edit', $role) }}" class="rounded-lg border border-gray-300 px-3 py-2 text-sm text-gray-700 hover:bg-gray-50 dark:border-gray-700 dark:text-gray-300 dark:hover:bg-white/[0.05]">Ubah</a>
                            @if ($role->slug !== 'super-admin' && $role->users_count === 0)<form method="POST" action="{{ route('roles.destroy', $role) }}" onsubmit="return confirm('Hapus role ini?')">@csrf @method('DELETE')<button class="rounded-lg border border-error-200 px-3 py-2 text-sm text-error-600 hover:bg-error-50 dark:border-error-800/60 dark:text-error-400 dark:hover:bg-error-500/10">Hapus</button></form>@endif
                        </div></td>
                    </tr>
                @endforeach
            </tbody></table>
