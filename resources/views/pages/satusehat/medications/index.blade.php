    ...
    <div class="space-y-6">
        ...
        <div class="grid grid-cols-1 gap-4 sm:grid-cols-3">
            @foreach ([['key' => 'all', 'label' => 'Semua Data'], ['key' => 'pending', 'label' => 'Belum Terkirim'], ['key' => 'sent', 'label' => 'Sudah Terkirim']] as $item)
                <a href="{{ route('satusehat.medications.index', array_merge(['type' => $type], request()->except(['status', 'page']), ['status' => $item['key']])) }}"
        ...
        <x-common.component-card :title="'Filter '.$definition['label']">
            <form method="GET" action="{{ route('satusehat.medications.index', $type) }}" class="grid grid-cols-1 gap-4 lg:grid-cols-12">
        ...
        <x-common.component-card :title="'Daftar '.$definition['label']">
            <x-satusehat.send-tips :resource="match($type) { 'medication' => 'medication', 'request' => 'medication-request', 'dispense' => 'medication-dispense', default => 'medication-statement' }" />
            @if ($connectionError)
                <x-ui.alert variant="warning" title="Database eksternal tidak dapat diakses" :message="$connectionError" />
            @endif
            @php
                $keyFor = function ($row) use ($type) {
                    if ($type === 'medication') return $row->item_code;
                    if ($type === 'dispense') {
                        [$batch, $invoice] = array_pad(explode('|', $row->extra, 3), 2, '');
                        return implode('|', [$row->no_rawat, $row->event_date, $row->event_time, $row->item_code, $batch, $invoice]);
                    }
                    return implode('|', [$row->no_resep, $row->item_code, $row->no_racik]);
                };
                $selectable = $rows->filter(fn ($row) => blank($row->sent_id))->map($keyFor)->values();
            @endphp
            <form method="POST" action="{{ route('satusehat.medications.send', $type) }}"
                x-data="{ selected: [], selectable: {{ Js::from($selectable) }} }"
                @submit.prevent="$dispatch('satusehat-send', { form: $el, label: '{{ $definition['label'] }}', count: selected.length })">
                @csrf
                <div class="mb-4 flex flex-wrap items-center justify-between gap-3">
                    <p class="text-sm text-gray-500">Data terbaru ditampilkan lebih dulu. Maksimal 20 data per pengiriman.</p>
                    @if (auth()->user()->hasPermission('medications.send'))
                        <button :disabled="selected.length === 0" class="rounded-lg bg-brand-500 px-5 py-2.5 text-sm font-medium text-white disabled:opacity-50">
                            Kirim Terpilih (<span x-text="selected.length">0</span>)
                        </button>
                    @endif
                </div>
                            ...
                            <tbody>
                                @forelse ($rows as $row)
                                    ...
                                    <tr class="border-b border-gray-100 dark:border-gray-800">
                                        <td class="px-5 py-4"><input type="checkbox" name="medications[]" value="{{ $keyFor($row) }}" x-model="selected" @disabled($sent || ! auth()->user()->hasPermission('medications.send')) class="size-4 rounded disabled:opacity-40"></td>
                                        <td class="whitespace-nowrap px-5 py-4 text-sm">{{ $type === 'medication' ? '-' : \Illuminate\Support\Carbon::parse($row->event_date)->format('d/m/Y') }}<span class="block text-xs text-gray-400">{{ $type === 'medication' ? '' : $row->event_time }}</span></td>
                                        <td class="whitespace-nowrap px-5 py-4 text-sm font-medium">{{ $row->no_resep ?: '-' }}<span class="block text-xs font-normal text-gray-400">{{ $row->no_rawat ?: '-' }}{{ $row->no_racik !== '' ? ' / Racik '.$row->no_racik : '' }}</span></td>
                                        <td class="px-5 py-4 text-sm">{{ $row->nm_pasien ?: '-' }}<span class="block text-xs text-gray-400">{{ $row->no_rkm_medis ? 'RM '.$row->no_rkm_medis : '' }}</span></td>
                                        <td class="px-5 py-4 text-sm font-medium">{{ $row->item_name }}<span class="block text-xs font-normal text-gray-400">{{ $row->item_code }}</span></td>
                                        <td class="px-5 py-4 text-sm">{{ $row->mapping_code }}<span class="block text-xs text-gray-400">{{ $row->form_display ?: '-' }}</span></td>
                                        <td class="max-w-[280px] px-5 py-4 text-sm">{{ $row->quantity ?: '-' }}<span class="block line-clamp-2 text-xs text-gray-400">{{ $row->instruction ?: '-' }}</span></td>
                                        <td class="px-5 py-4 text-sm">{{ $row->source_status ?: ($row->item_status == 1 ? 'Aktif' : 'Tidak Aktif') }}</td>
                                ...
                                @empty
                                    <tr><td colspan="10" class="px-5 py-12 text-center text-sm text-gray-500">Tidak ada {{ $definition['label'] }} yang sesuai dengan filter.</td></tr>
                                @endforelse
                            </tbody>
                        </table>
                    </div>
        ...
        <x-satusehat.send-popup validation-key="medications" />
    </div>
@endsection
