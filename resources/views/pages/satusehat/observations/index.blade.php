    ...
    <div class="space-y-6">
        <div class="flex flex-wrap gap-2">
            @foreach (['lab-mb' => 'Lab MB', 'lab-pk' => 'Lab PK', 'radiology' => 'Radiologi', 'ttv' => 'TTV'] as $key => $label)
                <a href="{{ route('satusehat.observations.index', array_merge(['type' => $key], request()->except(['type', 'page']))) }}" class="rounded-lg border px-4 py-2 text-sm font-medium {{ $type === $key ? 'border-brand-500 bg-brand-50 text-brand-600 dark:bg-brand-500/10 dark:text-brand-400' : 'border-gray-200 text-gray-600 hover:border-brand-300 dark:border-gray-800 dark:text-gray-300 dark:hover:border-gray-700' }}">{{ $label }}</a>
            @endforeach
        </div>
        ...
        @if ($type === 'ttv')
            <div class="flex flex-wrap gap-2">
                @foreach ($ttvTypes as $key => $item)
                    <a href="{{ route('satusehat.observations.index', array_merge(['type' => 'ttv'], request()->except(['ttv_type', 'page']), ['ttv_type' => $key])) }}" class="rounded-lg border px-3 py-1.5 text-xs font-medium {{ $filters['ttvType'] === $key ? 'border-brand-500 bg-brand-50 text-brand-600 dark:bg-brand-500/10 dark:text-brand-400' : 'border-gray-200 text-gray-600 hover:border-brand-300 dark:border-gray-800 dark:text-gray-300 dark:hover:border-gray-700' }}">{{ $item['label'] }}</a>
                @endforeach
            </div>
        @endif
        ...
        <x-common.component-card :title="'Filter '.$definition['label']">
            <form method="GET" action="{{ route('satusehat.observations.index', $type) }}" class="grid grid-cols-1 gap-4 lg:grid-cols-12">
                ...
                <div class="flex items-end gap-2 lg:col-span-2"><button class="h-11 flex-1 rounded-lg bg-brand-500 px-4 text-sm font-medium text-white hover:bg-brand-600">Terapkan</button><a href="{{ route('satusehat.observations.index', $type) }}" class="inline-flex h-11 items-center rounded-lg border border-gray-300 px-3 text-sm text-gray-700 hover:bg-gray-50 dark:border-gray-700 dark:text-gray-300 dark:hover:bg-white/[0.05]">Reset</a></div>
            </form>
        ...
        <x-common.component-card :title="'Daftar '.$definition['label']">
            ...
            <form method="POST" action="{{ route('satusehat.observations.send', $type) }}" x-data="{ selected: [], selectable: {{ Js::from($selectable) }} }" @submit.prevent="$dispatch('satusehat-send', { form: $el, label: '{{ $definition['label'] }}', count: selected.length })">
                ...
                <div class="overflow-hidden rounded-xl border border-gray-200 dark:border-gray-800"><div class="max-w-full overflow-x-auto custom-scrollbar">
                    <table class="w-full min-w-[1500px]"><thead class="bg-gray-50 dark:bg-gray-900"><tr class="border-b border-gray-200 dark:border-gray-800">
