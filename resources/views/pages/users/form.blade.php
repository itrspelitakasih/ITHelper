...
@section('content')
    ...
    <x-common.component-card :title="$title">
        <form method="POST" action="{{ $user->exists ? route('users.update', $user) : route('users.store') }}" class="space-y-5">
            ...
            <div class="flex justify-end gap-3"><a href="{{ route('users.index') }}" class="rounded-lg border px-5 py-3 text-sm">Batal</a><button class="rounded-lg bg-brand-500 px-5 py-3 text-sm text-white">Simpan</button></div>
        </form>
